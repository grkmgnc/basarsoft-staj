...
function App() {
            ...
            .then(data => {
                ...
                if (data.data && Array.isArray(data.data)) {
                    ...
                    data.data.forEach(g => {
                        ...
                        // Özellikleri topluca ekle
                        feature.setProperties({
                            id: g.id,
                            type: g.type.toLowerCase(),
                            name: g.name
                        });
                        feature.setId(g.id); // OpenLayers'ın id sistemine de ata (isteğe bağlı)
                        vectorSource.current.addFeature(feature);
                    });
                    //// Polygonları birleştir
    ...
    // Mod değiştiğinde interaction'ları güncelle
    useEffect(() => {
        if (!map) return;
        ...
        if (mode === 'add') {
            ...
            // Select ile seçilen feature'ı highlight etmek için
            selectInteraction.on('select', (evt) => {
                const feature = evt.selected[0];
                if (feature) {
                    // Feature’a ID ve type set et
            ...
            modifyInteraction.on('modifyend', (evt) => {
                evt.features.forEach((feature) => {
                    ...
                    if (!id || !type) {
                        alert('Geometri ID veya tipi bulunamadı!');
                        return;
                    }
    ...
    // Liste popup'ında geometriye git
    ...
    // Harita üzerinde feature tıklama ile popup açma
    useEffect(() => {
        if (!map) return;
    ...
    // Harita üzerindeki feature'ları filtrele (sadece Sadece Görüntüle modunda)
    useEffect(() => {
        if (!vectorSource.current) return;
        if (mode !== '') {
            // Diğer modlarda tüm feature'lar görünsün
    ...
    return (
        <div>
            ...
            {/* Filtre kutuları sadece Sadece Görüntüle modunda */}
            {mode === '' && (
